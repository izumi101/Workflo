...
_Next up: Frontend board — `apps/web`: auth shell (login/register wired to the existing Auth module) + Kanban UI with `@dnd-kit` wired to `POST /issues/:key/move` (optimistic drag reorder, `rankBetween` reused client-side from `@workflo/shared` for instant local placement before the server round-trip confirms). Real-time (Socket.IO + Redis adapter, `issue.created/updated/moved/deleted` events broadcasting board changes to other connected clients) is the step after that._
