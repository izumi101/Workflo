...
_Next up: Kanban board — drag & drop (status change + true LexoRank-style midpoint rank reordering, replacing the current append-only rank helper). Real-time (Socket.IO + Redis adapter, `issue.created/updated/moved/deleted` events) is the step after that._
